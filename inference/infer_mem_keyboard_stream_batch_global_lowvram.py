...
def _free_cuda():
    """Release cached CUDA memory after moving a model off the GPU."""
    gc.collect()
...
def move_to(module, device):
    """Move a model / pipeline to `device`, freeing CUDA cache when offloading."""
    if module is None:
        return module
...
def run(args, prompt, pipe, vda_model, epoch, file_name, device, edited_index,
    ...
    if start_id > 0:
        ...
        print(f"[INFO] edited_index: {edited_index} start_id: {start_id} prompt:{prompt}")
        
        ########### continous rotate after global event ###########
        keyframes_indices = extract_keyframes_indices(
            coordinates=coordinates_bank, 
...
def get_caption(model, processor, image_path, device):
    ...
    # [LOW-VRAM] Move Qwen3-VL onto GPU only for the duration of captioning.
    ...
    # [LOW-VRAM] Offload back to CPU to free GPU memory for the diffusion pipeline.
    move_to(model, "cpu")
    return caption
...
def main():
    ...
    # [LOW-VRAM] Keep the auxiliary Qwen-Image-Edit pipeline on CPU by default;
    # it is only moved to GPU during an `edit` action, then moved back.
    pipe_edit = QwenImageEditPipeline.from_pretrained(args.qwen_edit_path).to(torch.bfloat16).to("cpu")
    ...
    if args.auto_prompt:
        # [LOW-VRAM] Load Qwen3-VL on CPU; move to GPU only during caption generation.
        # NOTE: do NOT pass device_map here — accelerate's dispatch hooks would
        # prevent the later `.to(device)` / `.to("cpu")` swaps used to save VRAM.
        model = Qwen3VLForConditionalGeneration.from_pretrained(
            args.qwen_vl_path, dtype="auto"
        )
        model = model.to("cpu")
        processor = AutoProcessor.from_pretrained(args.qwen_vl_path)

        args.prompt = get_caption(model, processor, f"{args.output_dir}/{file_name}/input_image.png", device)
    ...
    for action in actions_list:
        if action["type"] == "edit":
            ...
            with torch.inference_mode():
                # 仅在编辑时把 Qwen-Image-Edit 移上 GPU，用完移回 CPU
                move_to(pipe_edit, device)
                ...
                move_to(pipe_edit, "cpu")

            source_image.save(f"{args.output_dir}/{file_name}/source_image_id{edited_index}.png")
            args.prompt = get_caption(model, processor, edited_image_savepath, device)
            ...
            epoch, coordinates_bank, videos_bank, depths_bank = run(
                args, args.prompt, pipe, vda_model, epoch, file_name, device, edited_index,
                origin, direction, local_speed, local_angle, local_num_frames,
                coordinates_bank, videos_bank, depths_bank
            )
