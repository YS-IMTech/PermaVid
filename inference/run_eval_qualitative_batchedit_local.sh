...
for local_id in 0 2 1; do
    ...
    # 构造输出目录名称，包含语义类型
    OUTPUT_DIR="results_exp/Qualitative_14B_localgood_long_mixmem_${edit_type}"
    mkdir -p "$OUTPUT_DIR"
    ...
    for ((dev=0; dev<DEVICE_COUNT; dev++)); do
        (
            ...
            for job in "${jobs_for_dev[@]}"; do
                ...
                # preset_actions="w-49-0.05, s-49-0.05, edit: $raw_prompt, right-49-1.0, left-49-1.0, left-49-1.0"

                python inference/infer_mem_keyboard_stream_batch_local.py \
                    --device_id "$dev" \
                    --model_id "$MODEL_ID" \
