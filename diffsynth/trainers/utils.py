...
class ModelLogger:
    ...
    def save_model(self, accelerator, model, file_name):
        ...
        if accelerator.is_main_process:
            ...
            print(f"[INFO] save model to path:{path}")
            accelerator.save(state_dict, path, safe_serialization=True)


def safe_collate_fn(batch):
    # Filter out None samples (skipped in __getitem__)
    batch = [x for x in batch if x is not None]
    if len(batch) == 0:
        return None  # empty batch
    return batch[0]  # keep batch size = 1 (as before)
...
#             ## data keys: ['video', 'prompt', 'vace_video', 'vace_video_mask', 'vace_reference_image', 'poses', 'intrinsics']
#             if data is None:
#                 continue 
...
def launch_training_task(
    ...
    if args is not None:
        ...
        wandb_log_interval = getattr(args, "wandb_log_interval", 1)
    else:
        wandb_project = None
    ...
    # === wandb init (only main process, after accelerator) ===
    wandb_enabled = False
    if wandb_project and accelerator.is_main_process:
        try:
            ...
            wandb.init(
                ...
                name=wandb_name,
                entity=wandb_entity,
                config=args.__dict__ if args else {},
                dir=args.output_path if hasattr(args, "output_path") else "./",
                settings=wandb.Settings(start_method="fork"),
            )
            wandb_enabled = True
            print(f"✅ wandb logging enabled: {wandb.run.name} @ {wandb.run.url}")
        ...
        for data in tqdm(dataloader, desc=f"Epoch {epoch_id+1}/{num_epochs}"):
            ...
            with accelerator.accumulate(model):
                ...
                try:
                    ...
                    global_step += 1
                    print(f"global-step:{global_step} loss:{float(loss.item())}")
                    # === wandb log loss (only main process) ===
                    if wandb_enabled and accelerator.is_main_process and global_step % wandb_log_interval == 0:
                        # Gather loss across devices for accurate logging
                        wandb.log({
                    ...
                    print(f"⚠️ Skipping batch due to error: {type(e).__name__}: {e}")
                    continue
...
def wan_parser():
    ...
    #add
    parser.add_argument("--resume_from_checkpoint", type=str, default=None, help="Path to resume from (directory or .safetensors file). Supports full, LoRA, or partial checkpoints.")
