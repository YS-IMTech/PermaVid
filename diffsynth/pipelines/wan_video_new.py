...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_NoiseInitializer(), ###  noise
            WanVideoUnit_PromptEmbedder(),   ###  "context": prompt_emb
            WanVideoUnit_S2V(),
            WanVideoUnit_InputVideoEmbedder(), ### latents
            WanVideoUnit_ImageEmbedderVAE(),   ### y
            WanVideoUnit_ImageEmbedderCLIP(),  ### clip_feature
            WanVideoUnit_ImageEmbedderFused(), ### "latents": latents, "fuse_vae_embedding_in_latents": True, "first_frame_latents": z
            WanVideoUnit_FunControl(),         ### "clip_feature": clip_feature, "y": y
            WanVideoUnit_FunReference(),       ### "reference_latents": reference_latents, "clip_feature": clip_feature
            WanVideoUnit_FunCameraControl(),   ### "control_camera_latents_input": control_camera_latents_input, "y": y
            WanVideoUnit_CameraControl(),   ### "control_camera_latents_input": control_camera_latents_input, "y": y
            WanVideoUnit_SpeedControl(),
            WanVideoUnit_VACE(),             ### vace_context
    ...
    def training_loss(self, **inputs):
        ...
        print("------ training loss -------")
        print("inputs[input_latents]:", inputs["input_latents"].shape)
        print("inputs[noise]:", inputs["noise"].shape)
    ...
    def from_pretrained(
        ...
        if redirect_common_files:
            redirect_dict = {
                ...
                "Wan2.1_VAE.pth": "Wan-AI/Wan2.1-VACE-14B",
                "models_clip_open-clip-xlm-roberta-large-vit-huge-14.pth": "Wan-AI/Wan2.1-VACE-14B",
            }
    ...
    def __call__(
        ...
        inputs_shared = {
            ...
            "original_pose_height": original_pose_height,  "original_pose_width": original_pose_width,
            "camera_control_direction": camera_control_direction, "camera_control_speed": camera_control_speed, "camera_control_origin": camera_control_origin, "coordinates": coordinates,
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames):
        print(f"执行类: {self.__class__.__name__}")
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        print("input_latents:", input_latents.shape)
        if vace_reference_image is not None:
           
            if not isinstance(vace_reference_image, list):
                vace_reference_image = [vace_reference_image]
            f = len(vace_reference_image)
            vace_reference_image = pipe.preprocess_video(vace_reference_image) #bs, c, f, h, w 
            ...
            vace_reference_latents = vace_reference_latents.permute(2,1,0,3,4)
            
            input_latents = torch.concat([vace_reference_latents, input_latents], dim=2)
            print( "input_latents:", input_latents.shape, "vace_reference_latents:", vace_reference_latents.shape, "vace_reference_image:", len(vace_reference_image))
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "num_frames", 
                          "original_pose_height","original_pose_width",
    ...
    def process(self, pipe: WanVideoPipeline, height, width, 
                num_frames, 
                original_pose_height, original_pose_width,
                camera_control_direction, 
                camera_control_speed, 
        ...
        if camera_control_direction is None and coordinates is None:
            return {}
        ...
        # print('===================process camera control plucker embedding =================')

        # print("coordinates:", coordinates)

        camera_control_plucker_embedding = pipe.dit.control_adapter.process_camera_coordinates(
            camera_control_direction, num_frames, height, width, original_pose_height, original_pose_width, 
            camera_control_speed, 
            camera_control_origin,
            coordinates)
        ...
        print("camera_control_plucker_embedding:", camera_control_plucker_embedding.shape)
        print("control_camera_video:", control_camera_video.shape)
        print("control_camera_latents:", control_camera_latents.shape)
        ...
        if y.shape[1] != pipe.dit.in_dim - latents.shape[1]:
            ...
            y = y.unsqueeze(0)
            y = y.to(dtype=pipe.torch_dtype, device=pipe.device)
        print("control_camera_latents_input:", control_camera_latents_input.shape)
...
class WanVideoUnit_CameraControl(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "num_frames", 
                          "original_pose_height","original_pose_width",
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            if vace_video is None:
                ...
                vace_video = pipe.preprocess_video(vace_video) #[1, 3, 101, 480, 832]
                        
            if vace_video_mask is None:
                vace_video_mask = torch.ones_like(vace_video) #[1, 3, 101, 480, 832]
                ###### add (for first-frame guide video generation)
                vace_video_mask[:, :, 0] = 0
            else:
            ...
            vace_video_latents = torch.concat((inactive, reactive), dim=1)      #[1, 32, 26, 60, 104]
            
            vace_mask_latents = rearrange(vace_video_mask[0,0], "T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8) #[1, 64, 50, 60,104]
            print("vace_mask_latents raw:", vace_mask_latents.shape)
            # [1, 64, 26, 60, 104]
            vace_mask_latents = torch.nn.functional.interpolate(vace_mask_latents, size=((vace_mask_latents.shape[2] + 3) // 4, vace_mask_latents.shape[3], vace_mask_latents.shape[4]), mode='nearest-exact')

            print("vace_mask_latents interpolate:", vace_mask_latents.shape, vace_video_latents.shape)

            if vace_reference_image is None:
                print("vace_reference_image is None")
                pass
            else:
                if not isinstance(vace_reference_image,list):
                    vace_reference_image = [vace_reference_image]
                ...
                print("vace_reference_image preprocess vace:", vace_reference_image.shape,"bs, c, f, h, w:", bs, c, f, h, w)

                new_vace_ref_images = []
                for j in range(f):
                    new_vace_ref_images.append(vace_reference_image[0, :, j:j+1])
                vace_reference_image = new_vace_ref_images #len=10,  vace_reference_image[0]: [3, 1, 480, 832]
                
                
                # [10, 16, 1, 60, 104]
                vace_reference_latents = pipe.vae.encode(vace_reference_image, 
                                                         device=pipe.device, 
                                                         tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) 
                ...
                vace_video_latents = torch.concat((*vace_reference_latents, vace_video_latents), dim=2) #[1, 32, 36, 60, 104]

                vace_mask_latents = torch.concat((torch.zeros_like(vace_mask_latents[:, :, :1].repeat(1,1,f,1,1)), vace_mask_latents), dim=2) #[1, 64, 36, 60, 104]
            
            
            print("====================vace_video_latents:", vace_video_latents.shape, "vace_mask_latents:", vace_mask_latents.shape)
            vace_context = torch.concat((vace_video_latents, vace_mask_latents), dim=1) # [1, 96, 36, 60, 104]
...
class WanVideoPostUnit_AnimateVideoSplit(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("input_video", 
                                       "animate_pose_video", 
                                       "animate_face_video",
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # print("vace_context:", vace_context.shape, "x:", x.shape, "control_camera_latents_input22:", control_camera_latents_input.shape)
    
    #### padding control_camera_latents_input (align the reference_images nums)
    # control_camera_latents_input:[b,24,f,h,w]
    if control_camera_latents_input is not None and (control_camera_latents_input.shape[2] != x.shape[2]):
        padding_f = x.shape[2] - control_camera_latents_input.shape[2]
        control_camera_latents_input = torch.concat((torch.zeros_like(control_camera_latents_input[:, :, :1].repeat(1,1,padding_f,1,1)), 
                                                     control_camera_latents_input), dim=2)
    
    # print("new control_camera_latents_input22:", control_camera_latents_input.shape)
