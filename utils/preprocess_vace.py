...
def prepare_source(src_video, src_mask, src_ref_images, num_frames, image_size, device='cpu'):
    area = image_size[0] * image_size[1]

    vid_proc = VaceVideoProcessor(downsample=tuple([x * y for x, y in zip((4, 8, 8), (1, 2, 2))]),
                min_area=480 * 832,
                max_area=480 * 832,
                min_fps=16,
                max_fps=16,
...
src_video = "benchmarks/VACE-Benchmark/assets/examples/swap_anything/src_video.mp4"
src_mask = "benchmarks/VACE-Benchmark/assets/examples/swap_anything/src_mask.mp4"
src_ref_images = "benchmarks/VACE-Benchmark/assets/examples/reference_anything/src_ref_image_1.png,benchmarks/VACE-Benchmark/assets/examples/reference_anything/src_ref_image_2.png"
frame_num = 81

src_video, src_mask, src_ref_images = prepare_source([src_video],
                [src_mask],
                [None if src_ref_images is None else src_ref_images.split(',')],
                frame_num, (480, 832))


print("src_video:", len(src_video), type(src_video[0]), src_video[0].shape, src_video[0].min(), src_video[0].max())
print("src_mask:", len(src_mask), type(src_mask[0]), src_mask[0].shape, src_mask[0].min(), src_mask[0].max())
print("src_ref_images:", len(src_ref_images[0]), type(src_ref_images[0][0]), src_ref_images[0][0].shape, src_ref_images[0][0].min(), src_ref_images[0][0].max())