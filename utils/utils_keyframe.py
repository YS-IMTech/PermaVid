...
############## inference/train extract frames core function ###############
def extract_keyframes_indices(
    coordinates,
    reference_nums,
    memory_start,
    memory_end,
    ...
    height,
    width,
    invalid_indexes=None,  # add
    overlap_threshold=0.8,
    fast=True,
):
    poses = load_poses_from_coordinates(coordinates)
    for p in poses:
        if "c2w" not in p:
    ...
    # 构建原始 memory indices 范围
    raw_memory_indices = list(range(memory_start, memory_end))
    
    # 剔除 invalid_indexes（如果提供）
    if invalid_indexes is not None:
        invalid_set = set(invalid_indexes)
        memory_indices = [i for i in raw_memory_indices if i not in invalid_set]
    else:
        memory_indices = raw_memory_indices

    query_indices = list(range(traj_start, traj_end))

    # 安全检查：避免越界
    max_valid_index = len(poses) - 1
    memory_indices = [i for i in memory_indices if 0 <= i <= max_valid_index]
    query_indices = [i for i in query_indices if 0 <= i <= max_valid_index]

    if not memory_indices:
        print("❌ Memory index list is empty after filtering invalid indexes.")
        return None
    if not query_indices:
        print("❌ Query index list is empty.")
        return None
    ...
    print(f"Memory [{memory_start}, {memory_end}) → {len(memory_indices)} (after invalid filter)")
    print(f"Query  [{traj_start}, {traj_end}) → {len(query_indices)}")

    #### params ####
    N = min(reference_nums, len(memory_poses))
    
    num_samples = 128
    no_spatial_filter = False
    spatial_percentile = 95.0
    
    if fast:
        effective_num_samples = max(32, num_samples // 4)
    ...
    retrieved_indices = retrieve_reference_frames(
        ...
        aspect=aspect,
        full_poses=None,
        memory_indices=memory_indices,  # 注意：传入的是过滤后的索引
    ...
    if retrieved_indices is None:
        print("\n❌ No reference frame meets the threshold. Exit.")
        return None
    print(f"\n✅ Extract({len(retrieved_indices)}) memory reference index: {retrieved_indices}")
    return retrieved_indices

# ==============================================================================
# coordinates.txt loading
# ==============================================================================
...
def retrieve_reference_frames(
    ...
    # selected_indices = [i+memory_start for i in selected_indices]
    
    selected_indices = [memory_indices[i] for i in selected_indices]    
...
# ############## inference/train extract frames core function ###############
# def extract_keyframes_indices_v0(coordinates, 
#                               reference_nums,
#                               memory_start,
#                               memory_end,
#                               traj_start,
...
#     print(f"Memory [{memory_start}, {memory_end}) → {len(memory_poses)}")
#     print(f"Query  [{traj_start}, {traj_end}) → {len(query_traj)}")
...
def main():
    ...
    # parser.add_argument("--coordinates_txt", type=str, default="results_test/coordinates/rgb_pose.txt")
    # parser.add_argument("--memory-range", type=str, default="0:100")
    # parser.add_argument("--traj-range", type=str, default="801:900")
